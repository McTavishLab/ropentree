...
subtaxa_gbif_all <- vector(mode = "list")
...
for (i in 1:length(subtaxa_name)){
  ...
  for (name in subtaxa_name[[i]]){
    ...
    if(inherits(name_gbif, "gbif") & !is.null(name_gbif$data)){
      ...
      if(nrow(name_gbif$data)){
        ...
        colnames(name_gbif)<-c("species","Longitude","Latitude")
      }
    }
  ...
  # concatenate results for each taxon
  subtaxa_gbif_all <- c(subtaxa_gbif_all, subtaxa_gbif)
}
#use taxon names to name the list of results for all taxa
names(subtaxa_gbif_all) <- names(subtaxa_name)
```
